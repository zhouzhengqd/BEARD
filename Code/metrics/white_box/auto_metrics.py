...
def calculate_ae(input_data):
    ...
    # Log-normalized values (not used by default)
    log_clean = np.log1p(avg_clean_time)
    log_attack = np.log1p(avg_attack_time)
    log_max_attack = np.log1p(max_attack_time)
    # Avoid division by zero
    ...
    # Log-normalized AE (optional)
    # ae = (log_attack - log_clean) / (log_max_attack - log_clean)
