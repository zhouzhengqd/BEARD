...
def get_dataset(dataset, data_path):
    ...
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=256, shuffle=False, num_workers=24)
    trainloader = torch.utils.data.DataLoader(dst_train, batch_size=256, shuffle=True, num_workers=24)
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, step=-1, bsr=False, pooling=False, partial=None, attack=None, target_attack=True):
...
def evaluate_synset(it_eval, net, images_train, labels_train, trainloader, testloader, args, upsample=False, pooling=False, partial=None, aug=True, tqdm_bar=False, train_attack=None, test_attack=None, src_dataset=False, target_attack=True):
    net = net.to(args.device)
    ...
    for ep in training_range:
        if not upsample:
            loss_train, acc_train = epoch('train', trainloader, net, optimizer, criterion, args, aug = aug, pooling=pooling, attack=train_attack, target_attack=target_attack)
        else:
            loss_train, acc_train = epoch_upsample('train', trainloader, net, optimizer, criterion, args, aug = aug, attack=train_attack, target_attack=target_attack)
        ...
        if tqdm_bar and 'imagenet_subset' not in args.dataset:
            ...
            loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, args, aug = False, partial=partial, attack=None, target_attack=target_attack)
            print("Test Attack Type:"+"None")
            print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test))
    print("Train Attack Type:"+train_attack)
    time_train = time.time() - start
    loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, args, aug = False, partial=partial, attack=test_attack, target_attack=target_attack)
    print("Test Attack Type:"+test_attack+str(target_attack))
    print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test))

    save_path = os.path.join(args.save_path, f"{args.method}_{args.model}_{args.dataset}_{args.ipc}_{args.test_attack}_{acc_test}.pth")
